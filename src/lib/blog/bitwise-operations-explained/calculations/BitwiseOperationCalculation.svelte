...
<div class="flex flex-col gap-8 overflow-x-auto font-mono text-4xl text-th-primary">
  <NumberGrid rows={4} cols={digits}>
    {#each bin1Padded as _, i}
      <div class="pt-2.5 text-sm text-th-secondary">2<sup>{bin1Padded.length - (i + 1)}</sup></div>
    {/each}
    {#each bin1Padded as char, i}
      <div
        class={char === '1' && bin2Padded[i] === '1'
